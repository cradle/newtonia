...
void Overlay::touch_controls(const GLGame *glgame, const GLShip *glship) {
  ...
  if(tc.joy_active) {
    ...
    glColor4f(0.4f, 0.5f, 1.0f, 0.35f);
    ...
    glColor4f(0.6f, 0.7f, 1.0f, 0.75f);
    draw_circle(bx + nx_off, by + ny_off, jr * 0.38f, 32, true);
  } else {
    // Inactive: faint hint ring at home position
    glColor4f(0.3f, 0.4f, 0.9f, 0.20f);
    draw_circle(jox, joy, jr, 32, false);
    // Small centre dot
    glColor4f(0.3f, 0.4f, 0.9f, 0.15f);
  ...
  // ---- Shoot button ----
  {
    ...
    float alpha_fill    = tc.shoot_pressed ? 0.40f : 0.12f;
    float alpha_outline = tc.shoot_pressed ? 0.75f : 0.35f;
    glColor4f(0.9f, 0.3f, 0.3f, alpha_fill);
    draw_circle(bx, by, br, 28, true);
    glColor4f(0.9f, 0.3f, 0.3f, alpha_outline);
  ...
  // ---- Mine button ----
  {
    ...
    float alpha_fill    = tc.mine_pressed ? 0.40f : 0.12f;
    float alpha_outline = tc.mine_pressed ? 0.75f : 0.35f;
    glColor4f(0.3f, 0.5f, 0.9f, alpha_fill);
    draw_circle(bx, by, br, 28, true);
    glColor4f(0.3f, 0.5f, 0.9f, alpha_outline);
    draw_circle(bx, by, br, 28, false);
  }
#endif // __ANDROID__
}
