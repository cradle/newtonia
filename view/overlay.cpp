...
void Overlay::score(const GLGame *glgame, const GLShip *glship) {
  ...
  Typer::draw((Typer::original_window_width)/glgame->num_x_viewports()-40, Typer::scaled_window_height/glgame->num_y_viewports()-20, glship->ship->score, 20);
...
void Overlay::lives(const GLGame *glgame, const GLShip *glship) {
  Typer::draw_lives(Typer::original_window_width/glgame->num_x_viewports()-40, -Typer::scaled_window_height/glgame->num_y_viewports()+70, glship, 18);
}
...
void Overlay::temperature(const GLGame *glgame, const GLShip *glship) {
  glPushMatrix();
  glTranslatef(-Typer::original_window_width/glgame->num_x_viewports()+30, -Typer::scaled_window_height/glgame->num_y_viewports()+15, 0.0f);
  glPushMatrix();
  glScalef(30,30,1);
  ...
  if(glgame->players->size() < 2) {
    if((glgame->current_time/1400) % 2) {
      if(p1->is_alive() || p1->lives > 0) {
        Typer::draw_centered(Typer::original_window_width/2, Typer::scaled_window_height-20, "player 2 press enter to join", 8);
      } else {
    ...
    if(glship->controller == NULL) {
      if(glship->show_help) {
        Typer::draw_centered(-1*Typer::original_window_width/2, Typer::scaled_window_height-20, "hide controls with F1", 8);
      } else if ((glgame->current_time)/12000 % 2) {
        Typer::draw_centered(-1*Typer::original_window_width/2, Typer::scaled_window_height-20, "show controls with F1", 8);
      }
    }
  } else {
