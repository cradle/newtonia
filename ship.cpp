  ...
  for(size_t i = 0; i < bullets.size(); ) {
    ...
    if(object != NULL) {
      ...
      if(ast && ast->reflective) {
        // Use relative velocity (bullet vs asteroid) for back-tracing so the
        // correct entry point is found even when the asteroid moves into the bullet.
        Point rel_vel = bullets[i].velocity - object->velocity;
        Point rel_norm = rel_vel.normalized();
        float max_trace = ast->effective_radius() * 2.0f + 4.0f;
        WrappedPoint entry = bullets[i].position;
        for (float d = 1.0f; d <= max_trace; d += 1.0f) {
          WrappedPoint test(bullets[i].position.x() - rel_norm.x() * d,
                            bullets[i].position.y() - rel_norm.y() * d);
        ...
        // Only reflect if the bullet is moving toward the surface in the
        // asteroid's frame (dot < 0).  If dot >= 0 the asteroid overtook the
        // bullet from behind — just push the bullet out without flipping velocity.
