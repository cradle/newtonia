...
**Adding the local build as a non-Steam game (Linux):** the shipped depot runs inside Steam's sniper container, which supplies freeglut; a non-Steam shortcut gets no container, and under the **snap/flatpak Steam** the sandbox can't see the host's `/usr/lib` either, so the loader failed on `libglut.so.3` before `main()` (field, snap Steam, 2026-09-05 — Steam's own record of it is `~/snap/steam/common/.local/share/Steam/logs/console-linux.txt`, or `~/.local/share/Steam/logs/` on the native package). `make steam` on Linux therefore also copies the host `libglut.so.3` beside the binary (`STEAM_LOCAL_LIBS`, found through `ldconfig -p`), where the `$ORIGIN` rpath resolves it; SDL2/SDL2_mixer come from the Steam runtime Steam puts on `LD_LIBRARY_PATH`, and libdatachannel through its absolute `netplay-libs` rpath. Point the shortcut at `newtonia-steam` with the repo root as its start directory (assets are CWD-relative). If it still dies silently, run `ldd ./newtonia-steam | grep 'not found'` from a wrapper script launched by the shortcut — that lists exactly what the sandbox is missing.
