...
- [x] **Phase 8 — polish**: pause propagation, disconnect UX ("CONNECTION LOST"), generation banner, 1 s input dead-man switch, REJECT UX, native↔web cross-play checklist. **Landed 1a644cf** — pause syncs via EV_PAUSE/EV_RESUME (host polls before its pause gate so remote RESUME lands while paused), CONNECTION LOST freeze+card on transport failure or EV_BYE, 2 s "LEVEL N - NEW X" banner via EV_GENERATION_START, dead-man releases held actions after 1 s of input silence, REJECT UX was in since Phase 5. **Remaining M1 verification: native↔web cross-play run.**
