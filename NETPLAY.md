...
- [ ] **Phase 0 — spike**: prove FetchContent MbedTLS+libdatachannel, USE_MBEDTLS=ON, NO_MEDIA/NO_WEBSOCKET/NO_EXAMPLES/NO_TESTS, /MT static CRT, MSVC; link + run a C-API loopback (also validates `rtcDataChannelInit`/reliability struct). **Spike written, NOT yet run** (see session log).
