...
- **2026-07-04 (g)**: **Phase 7 landed (fac5a77) — the milestone. Two machines play online co-op, native and web.**
  ...
  - **Mac native testing** (Glenn): `brew install libdatachannel` then `make NETPLAY=1 NETPLAY_PREFIX=$(brew --prefix)` — run two copies, ONLINE → HOST/JOIN, codes via cmd-C/cmd-V between them. Also cross-plays with web + Windows. The GLUT build inits SDL video under NETPLAY for the clipboard API.
