...
11. **New .cpp files must be added to `ios/Newtonia-iOS.xcodeproj/project.pbxproj`** — every other build discovers sources automatically (Makefile/web/iOS-CI/macOS-CI glob or `find`; Android/Xbox CMake `GLOB_RECURSE`), but the Xcode project lists files explicitly. iOS CI's main compile uses `find`, but the "Xcode project netplay compile gate (deploy-ios parity)" step in `ios.yml` DOES build through the project, so a broken pbxproj now fails CI (not just local Xcode). Add each new `.cpp` (headers too, for navigation) to all four sections: PBXBuildFile, PBXFileReference, the Sources group children, and the PBXSourcesBuildPhase, following the existing `AA…`/`AB…` ID scheme. **IDs must be globally unique** — when merging master, its new pbxproj entries (e.g. `hazard.cpp` at `AA000064`) can collide with this branch's netplay-file IDs (`AA000064` = `net_signal_web.cpp`, `AA000065` = `net_transport.cpp`, …). A duplicate ID makes Xcode resolve it to the wrong object — e.g. `net_transport.cpp`'s build-file pointed at `hazard.h`, so it silently stopped compiling and the link died on undefined `NetTransport::create`. After any master merge, `grep -oE 'A[AB]0000[0-9A-F][0-9A-F] ' project.pbxproj | sort | uniq -d` must be empty; re-number the merged-in entries into free IDs (the highest netplay IDs are around `A5`) if it isn't.
