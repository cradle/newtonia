...
- **TOUGH ELITES** spawn from generation ≥ 16 (`add_asteroids`, counts scale like the other specials): asteroids carrying TOUGH plus a second special — armoured+tough, teleporting+tough, quantum+tough, the lead combo rotating with the generation. All killable with the plain gun (reflective is excluded because its ctor forces `invincible`; invisible+teleporting was cut for readability); no savegame/protocol work — combos are just flag pairs the existing capture/restore already round-trips
