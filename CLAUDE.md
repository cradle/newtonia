...
**Lifetime stats** (`stats.h/cpp`) — standalone roaming `stats.dat` in the SDL pref path (magic "NWST", version 2, append-only format like the savegame): lifetime asteroid kills, a special-type kill mask, and shots fired (v2 — primary DISCHARGES only: scatter = 1, each burst emission = 1, beam/lance/shock bolts count, turret bullets and secondaries deliberately don't, so the STATS screen's ACCURACY line is the pilot's own trigger; counted at each weapon's post-ammo-check fire point, gated `is_local_player` + the cheat freeze like the kill counters — replay playback can't inflate it, NetReplay ships never run weapon sims), deliberately outside `savegame.dat` so netplay still counts. Kill writes are batched (every 10) and flushed via `Stats::flush()` from `save_progress()` and game over. Writes are skipped while the cheat flag is set — `specials_7`/`kills_10000_lifetime` read this file, so cheat games banking lifetime progress would launder achievements. Already in the Steam Auto-Cloud patterns (confirmed in the portal 2026-07-18), so it roams across installs.
