...
| `.github/workflows/android.yml` | Debug APK + debug-signed minified release APK (`-PdebugSign`; uploaded as `newtonia-release-minified` for device testing; both sign with the shared debug keystore decoded from the `DEBUG_KEYSTORE_BASE64` secret — one stable, PGS-registered cert across runs, falling back to a throwaway per-run cert without it; the keystore is secret-held, never committed — PLAYQUALITY.md), both booted through the emulator TLS gate (api-30 x86_64 AVD runs `NEWTONIA_SIGNAL_SELFTEST` via `android/emulator_selftest.sh`) — the release pass is the R8 boot gate (PLAYQUALITY.md) |
