...
**Particles** — `Particle` (`particle.h/cpp`): `Object` subclass with TTL countdown; `GLTrail` (`gltrail.h/cpp`) renders thruster particle trails. A particle can be flagged `kills_invincible` to penetrate shields. **Player-visible effects draw TRIANGLES, never GL_POINTS** (the ember-diamond pattern in `Hazard::draw` — velocity-streaked quads with the alpha-hold fade, reused by ship/station/mini-station debris and the exhaust trail): a field GPU (maintainer's Linux laptop, 2026-08-23) silently drops whole GL_POINTS draws under late-game load — trail vanishing mid-thrust, death and station explosions missing — while a Mac and the SAME machine under `LIBGL_ALWAYS_SOFTWARE=1` render every one of them, so it is the hardware driver, unfixable from the game, and other players will have that driver too. The GL_POINTS draws still in the codebase (starfield, asteroid debris/sparkles, bullet smoke `bullet_trails`, comet tail, minimap dots, Typer's dot glyphs) are ones that machine demonstrably renders — if any of them ever "stops working" in the field, convert it to the ember pattern rather than debugging the driver.
