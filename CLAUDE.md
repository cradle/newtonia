...
The project has been narrowed to a single active GitHub Actions workflow for the automatic PR/push build matrix — the Xbox build. Every other build/deploy workflow has been disabled (moved into `.github/workflows/disabled/`). One exception: `.github/workflows/windows-glon12.yml` is a GDKX-free rendering spike (`xbox/GLON12_SPIKE.md`) that runs only on the `claude/glon12-spike-**` branch or on manual dispatch, so it stays out of the regular matrix.
