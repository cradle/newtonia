...
- **Rammers** join the waves from generation ≥ 20 (`GLStation::wave_rammers`: 1 + (generation − 20), taking the slots the interceptor vanguard and the reserved standard leave AHEAD of the bombers so the debut shows by wave 2; they deploy right behind the vanguard). A crimson slim barbed spear (red-means-ram vocabulary from the seeker and hunter; densest trail in the wave palette): `Follower::RAMMER` carries no gun and NEVER breaks off — it steers at the intercept point and thrusts until it connects, the ram killing both hulls through the ordinary `Ship::collide` physics. Thrust 0.140–0.148 — between the standard ship and the interceptor, well under the player's 0.2 (the hunter's precedent), so a committed straight line always escapes; inside a firing wave the threat is the attention it taxes. Fourth `GLEnemy::Variant`, fully thrust-band encoded like the others — no savegame or PROTO change
