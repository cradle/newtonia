...
- `Hazard` obstacles fill the otherwise-quiet mid-game levels (`hazard.h/cpp`, spawned by `GLGame::add_hazards()`, counts scale with generation like the specials): pulsar ≥ 9, comet ≥ 11, seeker ≥ 12, hunter ≥ 18 (the seeker's elite: bigger, `HUNTER_HEALTH` = 3 shots, and it steers at the player's INTERCEPT point via the shared `seek.h` solver instead of tail-chasing — its speed stays under the player's plain thrust per the locked design rule; appended `Hazard::Kind` value, gated by savegame v22 + PROTO 28 with no record change — `health` already rode for the comet)
...
- **TOUGH ELITES** spawn from generation ≥ 16 (`add_asteroids`): asteroids carrying TOUGH plus a second special — armoured+tough, teleporting+tough, quantum+tough, one of EACH per set so every elite level carries all three, sets scaling like the other specials; the gen-16 intro shows the three in a row on the multi-rock asteroid display. All killable with the plain gun (reflective is excluded because its ctor forces `invincible`; invisible+teleporting was cut for readability); no savegame/protocol work — combos are just flag pairs the existing capture/restore already round-trips
- **Interceptors** join the station's waves from generation ≥ 15 (`GLStation::wave_interceptors`: 1 + (generation − 15) per wave, capped at half the wave; they deploy FIRST as a vanguard). An amber swept-dart wave ship with a denser trail, ~0.85x the player's plain thrust and a 1500 ms fire window (vs the standard 3000) — see the late-game design rule below
- **Bombers** join the waves from generation ≥ 17 (`GLStation::wave_bombers`: 1 + (generation − 17), capped by what the interceptor vanguard leaves minus one standard slot RESERVED in waves of 3+ ships — the specialist allotments otherwise swallowed every gen-19 wave up to size 7 and the green line vanished until wave 8; waves of 1–2 skip the reservation so the intro's promise still shows up by wave 2; they deploy LAST, the rear line). A violet squat hexagon barge (mortar tube at the prow, sparse trail, slowest hull in the game at ~0.105–0.115): `Follower::BOMBER` mode stands off (thrust cuts inside 700, engages from 900) and lobs mortar shells (`Ship::fire_bomb`) on a 4000 ms cadence instead of shooting. The shell is an ordinary bullet flagged `Particle::is_bomb` (drawn as a bright diamond + smoke trail; bit4 of the wire flag byte so clients render it too), fuse-detonated by a host-only pass in `GLGame`'s step loop: at end of flight (600 units) OR a living player within `BOMB_PROX_RADIUS` (90, armed after 300 ms so a point-blank charge doesn't eat a muzzle burst) it bursts into `BOMB_FRAGMENTS` (8) plain bullets at 0.3 u/ms in a random-phase ring — flak the player weaves through with plain thrust, per the design rule; fragments/shells replicate as ordinary enemy bullets (snapshot enemy-bullet records grew a v23-gated per-bullet flags byte, PROTO 29). Dev hook: `NEWTONIA_TEST_BOMBER=1` (beta builds) spawns one beside the player at game start, marked as a cheat
- **The station hardens** from generation ≥ 19 (the ARMOURED STATION): a bright cyan shield arc — the armoured-asteroid vocabulary at boss scale — rides the outer ring's rotation and deflects bullets (white ricochet + ting, host-side via `GLStation::deflects`, client-predicted in `net_cosmetic_ship_impacts` and reconciled by MSG_BOUNCE), missiles (detonate harmlessly), shock bolts (absorbed at the tip, spark) and lance pulses (absorbed at the contact point); rams damage through it. The ~120° gap is the weak window (coverage 240° + 8°/gen, capped at 300° so the plain-gun window never closes); health stays 100 — the difficulty is timing, not sponge. Destroying an armoured station pays `GLStation::REWARD` (2500) to the killing ship at every kill site (plain gen-14..18 stations stay achievement-only). Zero new persisted or replicated state: presence derives from `generation` (station rebuilt per rollover; save ctor and net replica both pass it), the arc angle from `outer_rotation` (saved + replicated since v1), so no savegame or PROTO bump
- **Rammers** join the waves from generation ≥ 20 (`GLStation::wave_rammers`: 1 + (generation − 20), taking the slots the interceptor vanguard and the reserved standard leave AHEAD of the bombers so the debut shows by wave 2; they deploy right behind the vanguard). A crimson hammerhead — wide swept head bar leading, body tapering to a thin tail, all the mass at the front because the front is the weapon (red-means-ram vocabulary from the seeker and hunter; densest trail in the wave palette): `Follower::RAMMER` carries no gun and NEVER breaks off — it steers at the intercept point and thrusts until it connects, the ram killing both hulls through the ordinary `Ship::collide` physics. Thrust 0.140–0.148 — between the standard ship and the interceptor, well under the player's 0.2 (the hunter's precedent), so a committed straight line always escapes; inside a firing wave the threat is the attention it taxes. Fourth `GLEnemy::Variant`, fully thrust-band encoded like the others — no savegame or PROTO change
- Pickups are cleared, the starfield and grid are rebuilt, players respawn, and progress is auto-saved
- Every level that introduces a new object type gets an intro screen — the `Intro` state (`intro.h/cpp`; asteroid specials at 1–8, pulsar at 9, mini-station at 10, comet at 11, seeker at 12, black hole at 13, station at 14, interceptor at 15 — that one shows an intro-owned display hull, since the real ones deploy mid-level; tough elites at 16, back on the asteroid display machinery; bomber at 17, on the interceptor's display-hull machinery (cruising slower, as a barge should); hunter at 18, focusing the live hazard like the other hazard intros; armoured station at 19, back on the station intro's live-object camera — the arc and gap are visible while it spins; rammer at 20, on the interceptor's display-hull machinery; a new game starts straight into play): the world freezes and the object spins centre-screen with its name and a flashing "PRESS FIRE TO START" ("TAP FIRE TO START" in touch mode); any player's shoot input (key, controller A/right trigger, the touch fire button — not a tap anywhere) dismisses it, and the touch OSD is drawn on the intro so the fire button is findable, or it auto-starts after 5 s (`Intro::auto_start_ms`) with no input (`GLGame::maybe_start_intro()` decides whether one is due and hands the game to a new `Intro`; not persisted in saves — resuming a save never re-shows one); a looping title-style tune (`audio/intro.wav`) plays on its own channel while other sound channels stay paused, halted on dismissal
...
| `weapon/missile` | Missile | Homing AI; forward ±45° cone seek every 8 ms step through the shared `seek_nearest` loop (`seek.h`) — asteroid candidates from `Grid::query_radius`, ships from the list `Ship::step` passes in (`GLGame::ship_objects`, which the station's deployed wave ships join at spawn and leave on every deletion path — so missiles home on enemies) |
...
**Debug cheats** — `NEWTONIA_ALL_WEAPONS=1` grants every primary (all default variants + Beam + Lance + Shock) and every secondary at 999 rounds on spawn and after each respawn (`Ship::give_all_weapons`, re-granted from `GLGame::tick`; a numeric value > 1 sets the rounds instead, e.g. `NEWTONIA_ALL_WEAPONS=30` for quick drain tests). `NEWTONIA_GOD=1` keeps every player permanently invincible — the plain `time_left_invincible` spawn-protection timer topped up per tick from `GLGame::tick`, so the flicker/ring visuals show the state and respawns can't strip it (NOT the god-mode weapon: no shockwaves, no music; the black hole's horizon still kills through it). Both flag the game as cheated (`Achievements::note_cheat_used()` in both `GLGame` constructors) so no achievements or lifetime stats count, and that flag rides the savegame (`GameState::cheated`) so save/resume can't launder it. Beta-gated spawn hooks (fresh game only, also marked as cheats): `NEWTONIA_TEST_BOMBER=1` spawns a lone bomber beside the player; `NEWTONIA_TEST_STATION=N` spawns a station built for generation N parked 900 right of the player (N=1 or bare means 19, the armoured one; 14–18 gives the plain station; skipped when `NEWTONIA_START_GENERATION` ≥ 14 already built the real, drifting one) — level-scoped, so clearing the opening level sweeps it.
...
| `HUNTER` | From generation ≥ 18: the seeker's elite. Bigger (radius 30), soaks `HUNTER_HEALTH` (3) shots (`hit()`, like the comet) for `HUNTER_REWARD`, and steers at the player's intercept point (shared `seek.h` solver at full lead, horizon-capped so an un-catchable target degrades to a plain chase). Speed 0.155 — under the player's plain 0.2 per the locked design rule. Rams like a seeker (destroying itself); crimson barbed triple diamond whose core blink quickens with damage taken — the health readout |

Serialization: `capture_state()` / `from_state()` (kind, position, velocity, shockwave phase). Destroyed seekers aren't persisted.

### Rendering

**OpenGL compatibility** — `gl_compat.h` and `gles2_compat.h/cpp` abstract desktop OpenGL 3.3 Core vs OpenGL ES 2 (mobile/web/Xbox). `gles2_compat` provides GLSL program wrappers, vertex/color buffers, and line-thickening for platforms where `lineWidth > 1` does nothing (WebGL clamps to 1, macOS core GL ignores it, ANGLE reports a [1,1] range). Real GLES drivers (Android/iOS) rasterize wide aliased lines natively, so `gles2_init` queries `GL_ALIASED_LINE_WIDTH_RANGE` and in-range widths bypass the emulation entirely — the per-draw CPU quad expansion + shared-VBO re-upload was the dominant mobile frame cost (~44 ms/frame in the objects pass at 21 fps, fixed by the bypass). `NEWTONIA_LINE_EMULATION=1` forces the emulation back on for A/B. The emulated stroke's solid-core fraction (`s_line_core_scale`) is measured in physical buffer pixels: desktop keeps the legacy thin 0.1, while web derives it from `devicePixelRatio` at startup and on resize (`web_main.cpp` `apply_line_core_scale`) — 0.1 at dpr 1 so a desktop-monitor browser matches the native build (the old fixed web 0.5 drew ships/asteroids ~2x fat there; field report Firefox/Linux 2026-08-19), ramping to 0.5 (conventional w-pixels, the native mobile aliased-line weight) from dpr 2 up. Typer pins its own 0.1 around text draws, so HUD/text weight never moves with this.
...
**Particles** — `Particle` (`particle.h/cpp`): `Object` subclass with TTL countdown; `GLTrail` (`gltrail.h/cpp`) renders thruster particle trails. A particle can be flagged `kills_invincible` to penetrate shields. **NOTHING draws GL_POINTS — every point-like effect is GL_TRIANGLES** through the two `MeshBuilder` helpers: `ember(x, y, vx, vy, len, wid)` (a diamond faintly streaked along its velocity — debris/sparks: hazard bursts + comet tail, ship/station/mini-station death bursts, asteroid sparkles) and `dot(x, y, r, z)` (axis-aligned — stars, exhaust puffs, bullet smoke, missile trails, turret hubs, minimap dots). The reason is a field GPU (maintainer's Linux laptop): on 2026-08-23 it silently dropped the debris/trail point draws under late-game load, and on 2026-08-30 it started dropping whole starfield layers every few seconds (foreground stars blinking in and out) — while a Mac and the SAME machine under `LIBGL_ALWAYS_SOFTWARE=1` rendered everything, so it is the hardware driver, unfixable from the game, and other players will have that driver too; that second report is what converted the holdouts, so don't reintroduce a point draw for a new effect. Point size lives in the geometry now, in the space the mesh is built in: star quads are depth-compensated per layer at build time (`GLStarfield::STAR_RADIUS_PER_DIST` × camera distance; the ctor's `camera_z` is 1000 for the game, 0 for the menu/lobby cameras) so every layer projects at the old fixed-pixel size; minimap dots are world-proportional (`minimap_dot_radius` in `glship.cpp`, over `WrappedPoint::x_span()`) because the minimap ortho spans the whole world — a world-proportional radius holds its on-screen size as the world grows, which the old pixel-sized points did for free; Typer's '?' dot became the '.' glyph's baseline dash. The `Mesh`/gles2 point plumbing (`uni_ispt`, the `point_size` draw args) remains but has no callers.
...
`GLStation` (`glstation.h/cpp`) — inherits `Ship`; rotating ring station with `health` and per-wave difficulty that deploys waves of `GLEnemy` ships; `capture_state()` / `restore_state()` for save/load. From generation ≥ 19 it is ARMOURED (`armoured()`/`deflects()`/`arc_coverage_deg()`; see the generation-progression notes) — when editing the arc keep the DRAW and the TEST on the same convention: the mesh is built centred on local angle 0 and drawn at `outer_rotation`, and `deflects()` measures impact bearing against that same `outer_rotation`, so the picture and the physics can never disagree.
...
**Late-game design rule (locked 2026-08-20)** — no post-14 escalation may require boost, a specific weapon, or any undiscovered mechanic as its ONLY counterplay: escapes must work with plain thrust on every input device, and every elite must die to the existing arsenal under existing rules (no INVINCIBLE in elite combos; readability is mandatory). Boost is now discoverable everywhere — a touch OSD button (amber double-chevron above the shoot/mine pair, native + web HTML) and a first-use "boost with e"/"boost with left bumper" hint on the F1 hint's alternate flash phase (retired for good by the first boost from any input path, `Preferences::boost_hint_done`) — and gated by `Ship::BOOST_COOLDOWN_MS` (2 s, inside `Ship::boost()` so keyboard, pad, touch, and the host's application of net clients' presses all share it; the OSD button dims while it runs) — but the rule STANDS: boost is comfort, never the only counterplay. Concretely for the interceptor: its thrust is capped ~0.85x the player's plain 0.2 — a committed straight-line escape always wins — and its threat is fire cadence plus the corner-cutting intercept steering, never a chase the player cannot break. For the bomber, the threat is area denial (slow, dodgeable flak rings), never speed — its shells fly at 0.25 and the fragments at 0.3, and the hull itself is the slowest in the game. For the armoured station, the shield arc never covers more than 300° — the plain-gun gap is permanent, and armour = rotating-weak-spot is vocabulary the player learned at gen 7. Wave ships never deliberately ram — except the RAMMER (gen ≥ 20), the one variant the reservation was for: everything else runs the `Follower` keep-distance attack-run hysteresis, and rams happen on momentum, never as the steady state. The rammer inverts the deal within the rule: no gun, never breaks off, but its 0.140–0.148 thrust sits under the standard escape bar, and its crimson hammerhead silhouette + densest trail make the charge readable at a glance. Wave-ship variants are fully encoded in stats `Save::Enemy` already carries (thrust/rotation/value), re-derived on restore/rebuild by `GLEnemy::variant_for_thrust` from non-overlapping thrust bands (bomber ≤ 0.12, standard ~0.129–0.135, rammer 0.140–0.148, interceptor ≥ 0.15 — keep the thresholds between the bands if any band moves) — no savegame record change; visuals are chosen at construction from the same variant.

**Hostile aim ramp** — the hostiles' guns (the mini-station's snipe, the station wave ships' `Follower` steering/shot gate) lead the player by a generation-scaled fraction of the perfect intercept: `GLGame::hostile_aim_lead(generation)` is 0 through the mini-station's gen-10 intro (first shots fly straight at you, as always) and climbs linearly to a full turret-style lead by generation 18 — the station's wave ships arrive at 14 already half-leading. The solve itself is the shared `intercept_offset()` in `seek.h` (extracted from the turret drone, which calls it at lead 1.0): earliest-positive-root constant-velocity intercept in the shooter's frame (bullets inherit 0.99 of shooter drift), returning the aim OFFSET to add to the target's position, (0,0) when no intercept exists inside the bullet's TTL, with `lead` scaling the intercept TIME so the ramp blends smoothly from aim-at-now to aim-at-arrival. Wired with **no saved or replicated state**: both stations are recreated on every generation rollover, so passing the value at construction tracks the generation, wave enemies inherit the station's value at deploy, and everything runs host-side (client replicas never steer or fire).

### Save / Load

**Savegame** (`savegame.h/cpp`) — binary format, magic "NWTN", version 23 (v22 carries no field changes — it gates the appended `Hazard::Kind` HUNTER value out of pre-hunter builds; v23 likewise carries none in the save body — it gates the snapshot/replay enemy-bullet records' appended per-bullet wire-flags byte, the bomber's shell glyph, out of v22 hunter replays that predate it):
- `WeaponEntry`: kind, weapon_index, ammo (kinds include the primaries `Beam`, `Lance`, and `Shock` — all captured/restored in the primary-weapon list; `Shock` appended in v15 after the branch's Beam/Lance to keep wire ordinals stable — and the `Turret` secondary, appended in v20; the deployed drones themselves persist too since v21 — see the Player entry)
- `Player`: score, lives, kills, respawning flag, position, velocity, facing, weapons, nova state, achievements bookkeeping (asteroid kills, enemy-ship kills, died-this-generation, weapons-fired mask; appended in v14 together with the game-scoped cheat flag), seat id (1..MAX_PLAYERS, appended in v19 with PROTO 25 — snapshots and the net restore paths key ship records by seat instead of list position; 0 in older files means positional, entry i = seat i+1), deployed turret drones (pos/vel, barrel angle, ms left, cooldown, shots — appended in v21 with PROTO 27 so a CONTINUE brings the battery back; `Ship::restore_state` gates the rebuild to offline/host-resume loads, since online the nx ship-extras stay the authoritative turret feed)
...
- Audio: `master_volume` / `music_volume` (0–1, default 1.0 — the AUDIO options sub-menu; pushed onto the mixer by `AudioVolume::apply()` in `audio_volume.h/cpp`: master rides SDL_mixer's channel master volume (2.6+, macro-guarded) + the music stream, music additionally scales the title stream and the intro/pause tune loops via `music_scale()` chunk-leveling at their play sites — the god-mode weapon music deliberately stays a gameplay cue under master only)
...
**Lifetime stats** (`stats.h/cpp`) — standalone roaming `stats.dat` in the SDL pref path (magic "NWST", layout v3 but the DISK stamp is pinned at 1: shipped v1 readers reject newer stamps and truncate-rewrite, so a bumped stamp meant one branch-switch or Auto-Cloud sync to an older install zero-wiped kills and specials_7 progress — appended fields are read by fread success instead, so the format must stay strictly append-only): lifetime asteroid kills, a special-type kill mask, shots fired (v2 — primary DISCHARGES only: scatter = 1, each burst emission = 1, beam/lance/shock bolts count, turret bullets and secondaries deliberately don't, so the STATS screen's ACCURACY line is the pilot's own trigger; counted at each weapon's post-ammo-check fire point), and the v3 block: enemy-ship kills, deaths, games played (fresh starts only, counted at the end of the fresh-game ctor AFTER every cheat determination, with a `NEWTONIA_SHOT` guard because the screenshot harness can only latch its cheat flag post-ctor), highest displayed level (max-tracked at the generation rollover + the client's `EV_GENERATION_START`, NetClient-gated so replayed event streams bank nothing), active play seconds (accumulated in `GLGame::tick` while running/not game over/not replay, written every ~60 s accrued), secondaries used (each secondary's post-ammo-check point via the shared `stat_secondary_used()` beside Ship; nova counts separately at `nova_detonate`), and novas detonated. Every counter shares one attribution gate — `is_local_player && !net_remote_gun` (replay playback marks ALL ships `net_remote_gun`, including the recorder's own seat, so watching can never bank anything) plus the cheat freeze — deliberately outside `savegame.dat` so netplay still counts. Kill writes are batched (every 10) and flushed via `Stats::flush()` from `save_progress()` and game over. Writes are skipped while the cheat flag is set — `specials_7`/`kills_10000_lifetime` read this file, so cheat games banking lifetime progress would launder achievements. Already in the Steam Auto-Cloud patterns (confirmed in the portal 2026-07-18), so it roams across installs.
...
maintainer is in **Melbourne (Australia/Melbourne)**: AEST UTC+10 in winter,
AEDT UTC+11 during daylight saving (first Sunday of October to first Sunday
of April) — `TZ=Australia/Melbourne date` gives the current offset. The
session container's clock, `run_once_at`, and `cron_expression` are all UTC,
which is exactly why every reported time has been wrong: quoting the
scheduling value back is off by ten or eleven hours. Convert before saying
anything user-facing — "I'll check back at 21:30 AEST", not "at 11:30".
Label the timezone. The reverse conversion (local → UTC) still applies when
*writing* a cron or `run_once_at`.
...
| `.github/workflows/ca-bundle-check.yml` | Weekly (Mon morning Melbourne time) staleness watch on the shipped CA roots: regenerates `net_ca_bundle.cpp` from curl.se's Mozilla bundle and, when the embedded certificates differ, goes red + files one tracking issue. Alert-only — the fix is a reviewed `./generate_ca_bundle.py` PR, never an auto-commit (it's the trust store). Ignores the header's date/sha stamp lines (`git diff -I`), so comment-only upstream edits stay green |
