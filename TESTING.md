...
| Android | MbedTLS | our bundle ONLY | ✅ `android.yml` emulator gate (was a real phone 2026-08-16, a manual UI drive before that) |
...
`NEWTONIA_NET_RTC`), and the SIGNAL one is now **CI-gated on an emulator** —
`android.yml`'s `emulator-selftest` job installs the debug APK on an
api-30 x86_64 AVD and runs `android/emulator_selftest.sh`, which is the
recipe below with retries, turning the log line into an exit status. That
script takes a real phone just as happily (`adb devices`), and
`SELFTEST=NEWTONIA_NET_SELFTEST` picks the loopback hook instead. The
emulator needs `/dev/kvm`, which exists on `ubuntu-latest` but is root-only
until the udev rule in the job opens it up.

The SIGNAL recipe below is device-verified on a real
...
**Re-arming: tested, does not happen.** A hook that stayed armed would
re-run the selftest on the next launch and close the app instead of
starting the game, so this was chased down rather than assumed.
Device-tested 2026-08-16, both routes: `am start` with the option and
then again without it, and — the case that could in principle have
replayed the task's retained intent — `am start` with the option followed
by a relaunch from RECENTS. Both start the game normally.

The native side clears both vars as it reads them, which covers a cached
process re-entering `SDL_main`; belt and braces after the above, and kept
because it costs nothing. No force-stop is needed.
...
`python3 test/unit/e2e_relay_startup.py`. It runs each of the six drivers
in an isolated fixture with fake relay commands and checks that a missing
game binary or failed `mktemp` exits before starting a relay. Load `lib.sh`
...
test/e2e/nseat_rejoin_flap_swap.sh # O4's seat-re-map case, which the
                     # single-parked-seat driver above cannot express. TWO
                     # seats parked, so the door offers seat 3 while seat 4's
