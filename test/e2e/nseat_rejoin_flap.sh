...
#     joined.
#
# NOT covered here: the case where the in-flight handshake belongs to a
# pilot heading for a DIFFERENT seat than the door offered (two seats
# parked, the higher seat's pilot answering the lower seat's offer —
# nseat_swap's shape). This driver parks ONE seat, so "same seat" and
# "same pilot" always answer alike here and it cannot tell a
# pilot-comparing resolver from a seat-comparing one. That case has its
# own driver, nseat_rejoin_flap_swap.sh, which needed a second hook to
# reach it: TEST_FLAP kills the socket, and the point there is a handshake
# that HANGS and then completes.
#
