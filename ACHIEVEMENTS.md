...
| iOS (Game Center) | this repo, `game_center_achievements.mm` | **implemented** — App Store Connect config pending (§2 checklist) |
...
2. Define **17 achievements** (all of §5 except `coop_clear`) with exactly
   ...
   Names/descriptions from the §5 table. All visible (none hidden),
   standard one-time achievements — Game Center needs no incremental
   configuration; progress is just the reported percent.
3. Points: Game Center caps a single achievement at 100 points (1,000
   ...
   **100** — total 900, leaving 100 headroom for `coop_clear` (40) at
   netplay time plus future additions.
...
5. Test with a development/TestFlight build (sandbox Game Center is
