...
**Steamworks portal checklist (no code) — COMPLETE (2026-07-26).** All four
steps are configured and published; the list below is kept as the record of
what was entered, and is what any future list change has to be replayed
against.
...
- **`coop_clear` is mapped** (post-netplay-release, task #153):
  `cc.gfm.newtonia.coop_clear` in the table, and **defined in App Store
  Connect (Co-Pilot, 40 pts) — done 2026-07-26**, bringing the portal total
  to 940. (Had the definition been missing, GameKit would reject the unknown
  identifier and the pending journal would retry later — no earns lost.)
...
3. Points: Game Center caps a single achievement at 100 points (1,000
   ...
   **100** — total 900, leaving 100 headroom for `coop_clear` (40) at
   ...
   `coop_clear` landed 2026-07-26, taking the total to **940**.
...
- **`coop_clear` is mapped** (post-netplay-release, task #154):
  `achievement_coop_clear` in the native table, **defined in the Play
  Console (Co-Pilot, one-shot, not incremental) and its generated ID wired
  into games-ids.xml — done 2026-07-26.** (Had the resource stayed absent,
  coop_clear earns would drop with a logcat warning; Play services queues
...
6. Add tester accounts under Play Games Services → Testers (separate from
   ...
   or the Play Games Management API. **Done 2026-07-26 — testers
   validated and the Games Services configuration published.**
...
achievements now that the hole sits at level 14 (see the earnability
audit).
