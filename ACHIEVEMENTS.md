...
6. Publish the Games Services configuration, add tester accounts under
   Play Games Services → Testers (separate from the app's internal-testing
   testers), and test with a debug build or the internal testing track:
   earns show as Play Games toasts. There is no client-side reset API —
   reset a test account's earns via the console or the Play Games
   Management API.
